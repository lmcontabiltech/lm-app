...
export class AuthService {
  ...
  obterPerfilUsuario(): Observable<Usuario> {
    return this.http.get<Usuario>(`${this.apiURL}/token`).pipe(
      ...
      catchError((error: HttpErrorResponse) => {
        ...
        return throwError('Erro ao obter perfil do usuário. Por favor, tente novamente.');
      })
    );
  }
  ...
  salvar(usuario: Usuario, perm: Permissao): Observable<any> {
    ...
    if (userPermissao === Permissao.ADMIN && perm === Permissao.COORDENADOR.valueOf()) {
  ...
  private cadastrarCoordenador(usuario: Usuario): Observable<any> {
    return this.http.post(`${this.apiURL}/cadastro/COORDENADOR`, usuario)
      .pipe(
        catchError((error: HttpErrorResponse) => {
          if (error.status === 422) {
            return throwError('Usuário já cadastrado na base de dados');
  ...
  public visualizarColaboradores(): Observable<Usuario[] | null> {
    return this.http.get<Usuario[]>(`${this.apiURL}/visualizar/COORDENADOR`, { observe: 'response' }).pipe(
      map((response) => {
